...
namespace Adrenak.UniVoice
{
	...
	public class ClientSession<T> : IDisposable
	{
		...
		public Dictionary<T, OutputAudioSettings> PeerAudioSettings = new();

		public OutputAudioSettings LocalAudioSettings = new();
			...
			set
			{
				...
				Client.OnPeerJoined += id =>
				{
					try
					{
						...
						PeerAudioSettings.Add(id, new OutputAudioSettings());
					}
					catch (Exception e)
					{
				...
				Client.OnReceivedPeerAudioFrame += (id, audioFrame) =>
				{
					...
					if (!PeerAudioSettings.TryGetValue(id, out settings))
					{
						settings = new OutputAudioSettings();
					}

					if (!OutputsEnabled || !PeerOutputs.ContainsKey(id) || settings.Muted)
						return;
					...
					{
						var sampleFloats = Utils.Bytes.BytesToFloats(audioFrame.samples);
						for (var i = 0; i < sampleFloats.Length; i++)
						{
							sampleFloats[i] *= settings.Volume;
						}
						audioFrame.samples = Utils.Bytes.FloatsToBytes(sampleFloats);
					}
		...
		public IAudioInput Input
		{
			...
			set
			{
				...
				input.OnFrameReady += frame =>
				{
					...
					{
						var samplesFloat = Utils.Bytes.BytesToFloats(frame.samples);
						for (var i = 0; i < samplesFloat.Length; i++)
						{
							samplesFloat[i] *= LocalAudioSettings.Volume;
						}
						frame.samples = Utils.Bytes.FloatsToBytes(samplesFloat);
					}

					if (InputFilters != null && frame.samples != null)
					{
